				...
				if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){		//判断登录验证码
					...
					if(pd != null){
						if(pd.getString("STATUS").equals("0")) {
							...
							//根据userid查询stroeid
							if(pd.getString("ROLE_ID").equals(Const.STROE_ADMIN_ROLE_ID)) {
								PageData queryStroeIdPd = new PageData();
								queryStroeIdPd.put("USER_ID", pd.getString("USER_ID"));
								List<PageData> list = businessService.findStoreByUserId(queryStroeIdPd);	//根据用户编号查询商户编号
								if(list != null && list.size() >= 0) {
									session.setAttribute("STORE_ID", list.get(0).getString("STOREID"));
								}
							}
