package com.flc.controller.coupon;
...
public class CouponController extends BaseController {
	...
	public ModelAndView save() throws Exception{
		...
		pd = this.getPageData();
		int count = Integer.parseInt(pd.getString("CREATE_NUM"));
		for(int i = 0; i < count; i++) {
			pd.put("COUPON_ID", this.get32UUID());	//主键
			pd.put("CREATE_TIME", DateUtil.getTime());
			pd.put("COUPON_NUM", Tools.getRandomNumAndLetter(10));	//优惠券码
			pd.put("STATUS", "0");
	...
	public ModelAndView list(Page page) throws Exception{
		...
		String ISYX = pd.getString("ISYX");
		if(null != ISYX && !"".equals(ISYX)){
			pd.put("ISYX", ISYX.trim());
		}
		page.setPd(pd);
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("coupon/coupon_edit");
	...
	public ModelAndView exportExcel() throws Exception{
		...
		List<PageData> varOList = couponService.list(page);	//列出Coupon列表
		List<PageData> varList = new ArrayList<PageData>();
		for(int i=0;i<varOList.size();i++){
			PageData vpd = new PageData();
			vpd.put("var1", varOList.get(i).getString("COUPON_NUM"));	    //1
			vpd.put("var2", varOList.get(i).getString("COUPON_MONEY"));	    //2
